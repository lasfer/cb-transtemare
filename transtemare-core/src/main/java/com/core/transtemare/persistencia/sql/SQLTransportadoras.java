...
public class SQLTransportadoras {
	...
	// Obtener
	public static final String OBTENER_RESPONSABLE_POR_ID = "SELECT CodResponsable , Nombre,Apellido,Documento,p.descripcion as pais,l.descripcion as ciudad, telefono FROM  responsable r , localidad l , pais p WHERE r.FK_Localidad = l.CodLocalidad AND l.CodPais_FK = p.CodPais AND CodResponsable = ?";
	...
	public static final String OBTENER_TODOS_LOS_CHOFERES = "SELECT r.CodResponsable , r.Nombre , r.Apellido , r.Documento , l.descripcion as ciudad, p.descripcion as pais, telefono FROM  responsable r,  localidad l ,  pais p WHERE r.FK_localidad = l.CodLocalidad AND l.CodPais_FK = p.CodPais AND r.Activo = 1 AND CodResponsable != 0 ORDER BY r.Apellido ASC";
	public static final String OBTENER_TODOS_LOS_CHOFERES_DESDE_HASTA = "SELECT r.CodResponsable , r.Nombre , r.Apellido , r.Documento , l.descripcion as ciudad, p.descripcion as pais, telefono FROM  responsable r,  localidad l ,  pais p WHERE r.FK_localidad = l.CodLocalidad AND l.CodPais_FK = p.CodPais AND r.Activo = 1 AND CodResponsable != 0 and (r.nombre like ? and r.apellido LIKE ?) ORDER BY r.Apellido ASC LIMIT ? , ?";
	...
	public static final String BORRAR_RESPONSABLE = "UPDATE  responsable SET Activo=0 WHERE CodResponsable=?";
	public static final String BORRAR_CAMION = "UPDATE  camion SET Activo=0 WHERE idCamion=?";
