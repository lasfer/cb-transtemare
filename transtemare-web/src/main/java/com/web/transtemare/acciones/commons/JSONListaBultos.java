...
public class JSONListaBultos extends ActionSupport {

	private Logger logger = Logger.getLogger(JSONListaBultos.class);
	...
	public JSONListaBultos(Fachada fac) {
		super();
		this.fac = fac;
	}

	@Action(value = "/jsonListaBultos", results = { @Result(type = "json", name = "success", params = {
			"root", "lista" }) })
	public String execute() {
		...
		try {
			fac.obtenerTodosLosBultos(bultos);
		...
		if (term != null && term.length() > 1) {
			for (int i = 0; i < bultos.size(); i++) {
				if (StringUtils.contains(bultos.get(i).getDescripcion()
						.toLowerCase(), term.toLowerCase())) {
					lista.add(bultos.get(i).toString());
				}
			}
		}
		return SUCCESS;
